...
playAudioSwitch.oninput = () => {
	if (playAudioSwitch.checked)
		custom
			.confirm("Attention", "For this feature to work, you need to have Autoplay enabled.", "OK", "no thanks")
			.catch(() => {
...
const game = {
	...
	draw: {
		...
		net(color) {
			for (let i = 0; i <= canvas.height; i += 15) {
				game.draw.Rect(net.x, net.y + i, net.width, net.height, color ? color : game.defaultColor);
			}
		},
		Text(text, x, y, color) {
			cvsContext.fillStyle = color ? color : game.defaultColor;
	...
	update() {
		...
		if (ball.y + ball.radius >= canvas.height || ball.y - ball.radius <= 0) ball.velocity.y = -ball.velocity.y;
	...
	pause() {
		...
		canvas.onclick = (event) => {
			if (!event.target.matches(".settings-panel")) if (!game.isRunning) game.start();
		};
	},
	stop() {
		clearInterval(game.Cycle);

...
function start_func() {
	...
	if (hour_input.value == "" && minute_input.value == "" && second_input.value == "") return;
	...
	display(hour, minute, second, false, 100);
	...
	if (checkColorchange.checked == true) bodyDiv.style.animation = "hueRotate " + intrinsicTime + "s linear";
	...
	intervalVar = setInterval(() => {
		if (totalTimeInSeconds == 0) Timeout_func();
			...
			display(hour, minute, second, true, progressPercentage);
		}
	}, 1000);
}
...
function Timeout_func() {
	...
	if (checkAlertText.checked) {
		let countUpInt;
		custom
			.confirm({ title: "Timeout!", text: alertTextInput.value }, ({ title }) => {
				const countUp = {
					s: 0,
					min: 0,
				};
				countUpInt = setInterval(() => {
					countUp.s++;
					if (countUp.s == 60) {
						countUp.s = 0;
						countUp.min++;
					}
					title.innerText = `Timeout since ${countUp.min > 0 ? countUp.min + "min" : ""} ${countUp.s}s!`;
				}, 1000);
			})
			.then((res) => {
	...
	if (checkBlinkText.checked == true) countdown.style.animation = "blink 2s linear";
...
//Output to inputfields and Countdown
function display(hour, minute, second, running, progressPercentage) {
	if (running == false) {

...
const timetable = {
	...
	fillWithData: (timetableWeek) => {
		...
		console.log(timetableWeek);
		timetableWeek.forEach((item, i) => {
			console.log("--------------------------");
			const timetableDay = item;
	...
	createElement: {
		...
		day: (timetableDay) => {
			...
			timetableDay.forEach((lesson) => dayElem.append(timetable.createElement.lesson(lesson)));

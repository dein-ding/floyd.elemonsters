(async () => {
	...
	const { sternzeichen, kategorieText } = await getFileAsync("/src/data/horoskopTexte.json", true);
	...
	function submit() {
		...
		if (
			...
			if ((mmdd >= 1222 && mmdd <= 1231) || (mmdd >= 101 && mmdd <= 120)) sternzeichenIndex = 11; //Wassermann
			...
			////////// Geburtstag /////////////
			if (validateInput(Geburtstag_input.value, "day")) Geburtstag_input.style.border = INPUT_DEFAULT_BORDER;
			else Geburtstag_input.style.border = INPUT_ERROR_BORDER;

			////////// Geburtsmonat /////////////
			if (validateInput(Geburtsmonat_input.value, "month")) Geburtsmonat_input.style.border = INPUT_DEFAULT_BORDER;
			else Geburtsmonat_input.style.border = INPUT_ERROR_BORDER;

			////////// Kategrorie //////////////
			if (validateInput(kategorie_dropdown.value, "dropdown")) kategorie_dropdown.style.border = INPUT_DEFAULT_BORDER;

$(document).ready(function () {
    ...
    if (screen.width <= 700) {
        ...
        custom
            .confirm(
                "Attention",
                "This site works and looks better on desktop devices, continue?",
                "continue",
                "cancel"
            )
            .then((response) => {
    ...
    if (isMobile)
        firstLiTag.innerHTML =
            "tap on colors to see their RGB values <i class='fa fa-eye-dropper'></i>";
    ...
    $("body").mousemove((event) => {
        ...
        if (
            event.target.dataset.description == "colorField" &&
            isMobile == false
        )
            pipetteCursor.style.opacity = 1;
        else pipetteCursor.style.opacity = 0;

        if (event.target.dataset.description == "addDivButton")
            document.querySelector("#dontShowAusgabeDiv").checked = true;
    ...
    function copyToClipboard(text) {
        ...
        clipboardMessage.style.animation =
            "clipboardMessage " + CLIPBOARD_MESSAGE_ANIMATION_DURATION + "ms";
    ...
    /* custom.confirm(
        "Test",
        "This app works and looks better on desktop devices, continue?",
        "continue",
        "cancel"
    ).then((response) => {
        switch (response) {
